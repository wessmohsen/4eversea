...
<div id="header-wrapper" class="<?php echo esc_attr($header_class); ?>">
    ...
    <?php
        if( !empty( $global_breadcrumb ) ) {
            ...
            $breadcrumbs[] = __('Hotels','designthemes-core');
            $breadcrumbs[] = '<a href="'. get_category_link( get_query_var('hotel_entries') ) .'">' . single_cat_title('', false) . '</a>';
...
<div id="main">
    <!-- ** Container ** -->
    <div class="container"><?php
        $page_layout = cs_get_option( 'hotel-archives-page-layout' );
        ...
        if ( $show_sidebar ) {
            if ( $show_left_sidebar ) {?>
                ...
                <section id="secondary-left" class="secondary-sidebar <?php echo esc_attr( $sidebar_class );?>"><?php
                    ...
                    if( is_active_sidebar('custom-post-hotel-archives-sidebar-left') ):
                        dynamic_sidebar('custom-post-hotel-archives-sidebar-left');
                    endif;

                    $enable = cs_get_option( 'show-standard-left-sidebar-for-hotel-archives' );
                    if( $enable ):
        ...
        if ( $show_sidebar ) {
            if ( $show_right_sidebar ) {?>
                ...
                <section id="secondary-right" class="secondary-sidebar <?php echo esc_attr( $sidebar_class );?>"><?php
                    ...
                    if( is_active_sidebar('custom-post-hotel-archives-sidebar-right') ):
                        dynamic_sidebar('custom-post-hotel-archives-sidebar-right');
                    endif;

                    $enable = cs_get_option( 'show-standard-right-sidebar-for-hotel-archives' );
                    if( $enable ):
